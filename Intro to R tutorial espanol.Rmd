...
#Cualquier objeto creado aqu? estar? disponible en el entorno global. Cualquier objeto creado dentro de este pedazo del ejercisio solo est? disponible dentro del pedazo.
...
# Introducción a R
En este tutorial, aprenderas c?mo manipular y trazar datos en R

## Comencemos mirando algunos datos reales
Estos datos de hospitalizaci?n a causa de neumon?a en ni?os <12 meses de edad en Brasil. Los datos est?n al nivel estatal, y cada columna le corresponde a uno de los 27 estados. Los datos est?n guardado como *matriz*, con 27 columnas y 144 filas, representando datos mensuales durante 12 a?os.
...
En R se puede operar con numeros individuales (es decir, calcular el valor de 1+1) con vectores de numeros (es decir, un variable), o con matrices. Los datos ademas se pueden guardar en marcos de datos que es como una matriz pero se puede guardar una mezcla de variables num?ricas y variables de caracteres.

Primero, extraiga una de las columnas de los datos de Brazil, crea un variable de ?ndice y juntalas en una nueva matriz. 
...
Vectores (variables) se pueden juntar en una matriz o un marco de datos. (Marcos de datos tiene el ventaja de poder incluir variables num?ricas y caracteres). Intente cambiar el c?digo para imprimir las primeras 20 filas de datos en vez de las primeras 10 filas de datos.
...
#Obtenga el n?mero de filas y columnas en 'mat1'
nrow(mat1)
```
...
Uno puede subcontratar la matriz haciendo referencia a los nombres o posici?n de columnas espec?ficas. Intente imprimir Brazil y imprimir las primeras 10 filas de datos en vez de 20 filas. 
...
Este c?digo reemplaza la primera columna de la matriz con un vector de observaciones. Intente reemplazar la segunda columna con los datos observados de estado 31. Adem?s intente usar 'rnorm' para generar n?meros al azar para columna 3. 
...
## Aritm?tica en R
Ahora, hagamos aritm?tica simple con estas variables, y combinarlas en una matriz.
...
Quizas adem?s quisiera agregar en todas las columnas. Se puede cumplir usando la funci?n 'rowSums' o con la funci?n 'apply' los cuales repite cada operaci?n por cada fila. Luego impreme las primeras 20 observaciones. 
...
timefill[1:144] #Imprimir los primeros a?os
```

Si tienes un vector de fechas en formato de car?cter, tienes que comandar R que es una fecha usando 'as.Date', y decirle cu?l es el formato. For ejemplo, mm/dd/yy tiene el formato "%m/%d/%y".
```{r dates2, exercise=TRUE, exercise.eval=FALSE}
#Si la fecha est? en formato mm/dd/aa
date1<-c("01/01/98", "02/01/98","03/01/98")
date2 = as.Date(as.character(date1), format = "%m/%d/%y")  #Tenga en cuenta que una Y min?scula coresponde a a?os con 2 d?gitos
date2

```

Aqu? la fecha est? en formato mm/dd/aaaa. Le decimos esto a R usando el formato "%m/%d/%Y". Tenga en cuenta que la Y may?scula coresponde a a?os con 4 d?gitos. 
```{r dates3, exercise=TRUE, exercise.eval=FALSE}
#Si la fecha est? en formato mm/dd/aaaa
date1<-c("01/01/1998", "02/01/1998","03/01/1998")
date2 = as.Date(as.character(date1), format = "%m/%d/%Y")  #Tenga en cuenta que una Y may?scula coresponde a a?os con 4 d?gitos
date2

```

Transformando fechas en R es flexible y puedes utilizar cualquier formato. Por ejemplo, aqu? tenemos una fecha en formato dd-mm-aa, y le decimos a R que est? en formato "%d-%m-%y". 
...
### Ahora vamos a cambiarle las opciones para crear trazados m?s bonitos
...
### Grafique las primeras 3 columnas, que representan 3 estados diferentes. Se cumple esto usando un bucle, que traza columna 1, luego columna 2, luego columna 3. Est?n organizados en una fila (mfrow=c(1,3)). Agregue un t?tulo extrayendo los nombres de las ubicaciones en un vector (dimnames) y tomando el elemento i'to del vector. 
...
Aqu? se puede producir 4 trazados en un 2x2 matriz. Intente producir un 2x3 trazado con 6 estados.
...
## RStudio
Ahora vamos a RStudio y vamos a familiarizarse con la interfaz y aprender c?mo configurar el directorio o 'working directory', y c?mo instalar y cargar paquetes.